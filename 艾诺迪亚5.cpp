...
int monsterATK[22]={0,0,345,375,375,385,0,0,405,465,505,555,615,675,705,745,785,835,865,915,935,1955};
...
void Train_skill()
{
	...
	Slowsay("请输入您想修炼的技能，r键返回           请注意：您还有");cout<<jinbi;Slowsay("金币\n");
	char ch3;
	cin>>ch3;
	if(ch3!='r'&&ch3!='1'&&ch3!='2'&&ch3!='3'&&ch3!='4')
	{
		goto TS;
	}
	if(ch3=='r')
...
bool zhandou(string guai,int p) {
	...
	{	
		...
		if(p==-5){guai="克拉尔";mHP=2000;mATK=300;}
...
youxi:
		...
		qsay("A,巨灵药水     瞬间回复满所有血与魔法值       ￥1000(店长推荐)\n");
		qsay("r,返回                   请注意：你现在还有");
		cout<<jinbi;
		Slowsay("金币\n");
		Sleep(1000);
		...
		if((ch4=='a'||ch4=='A')&&jinbi>=1000&&bag1<9) {
			yaoshuixiao[bag1]=10;
			jinbi-=1000;
			yaoshui[bag1]="巨灵药水";
			bag1++;
...
hujia:
		...
		qsay("2,铁制铠甲套装   防御力+105  职业：狂战士 or 圣骑士 ￥450\n");
		qsay("3,金制护甲套装   防御力+175  职业：法师 or 刺客     ￥550\n");
		qsay("4,轻型铠甲套装   防御力+295  职业：刺客             ￥1000\n");
		qsay("5,重型铠甲套装   防御力+975  职业：圣骑士           ￥3000\n");
		qsay("6,锁链护甲套装   防御力+420  职业：法师             ￥1600\n");
		qsay("7,钻石护甲套装   防御力+775  职业：狂战士           ￥3000\n");
		qsay("8,鳄鱼皮甲套装   防御力+380  职业：法师 or 刺客     ￥1200\n");
		qsay("9,束腰衬衫套装   防御力+210  职业：法师             ￥800\n");
		qsay("r,返回                                  请注意，你还有");
		cout<<jinbi;
		say("金币\n");
		Sleep(2000);
		...
		if(ch5=='2'&&jinbi>=450&&(juese=="狂战士"||juese=="圣骑士")&&bag<9) {
			jinbi-=450;
			wuqi[bag]="铁制铠甲套装";
			xinneng[bag]=105;
			wuqiLV[bag]=1;
		...
		if(ch5=='3'&&jinbi>=550&&(juese=="法师"||juese=="刺客")&&bag<9) {
			jinbi-=550;
			wuqi[bag]="金质护甲套装";
			xinneng[bag]=175;
			wuqiLV[bag]=1;
		...
		if(ch5=='4'&&jinbi>=1000&&juese=="刺客"&&bag<9) {
			jinbi-=1000;
			wuqi[bag]="轻型铠甲套装";
			xinneng[bag]=295;
			wuqiLV[bag]=1;
		...
		if(ch5=='5'&&jinbi>=3000&&juese=="圣骑士"&&bag<9) {
			jinbi-=3000;
			wuqi[bag]="重型铠甲套装";
			xinneng[bag]=975;
			wuqiLV[bag]=1;
		...
		if(ch5=='6'&&jinbi>=1600&&juese=="法师"&&bag<9) {
			jinbi-=1600;
			wuqi[bag]="锁链护甲套装";
			xinneng[bag]=420;
			wuqiLV[bag]=1;
		...
		if(ch5=='7'&&jinbi>=3000&&juese=="狂战士"&&bag<9) {
			jinbi-=3000;
			wuqi[bag]="钻石护甲套装";
			xinneng[bag]=775;
			wuqiLV[bag]=1;
		...
		if(ch5=='8'&&jinbi>=1200&&(juese=="法师"||juese=="刺客")&&bag<9) {
			jinbi-=1200;
			wuqi[bag]="鳄鱼皮甲套装";
			xinneng[bag]=380;
			wuqiLV[bag]=1;
		...
		if(ch5=='9'&&jinbi>=800&&juese=="法师"&&bag<9) {
			jinbi-=800;
			wuqi[bag]="束腰衬衫套装";
			xinneng[bag]=210;
			wuqiLV[bag]=1;
...
void Task()
{
	...
	switch(rate)
	{
		...
		case 1:{
			...
			Slowsay("“你知道刘邦吗？”\n\n");
			getline(cin,s);
			Slowsay("当然，就是那个击败西楚霸王项羽的巨佬呗\n");
			getline(cin,s);
			system("cls");
			Slowsay("说的没错，传说在始皇三十四年时，他从南山仙人处得一帝道之剑——赤霄\n\n");
			getline(cin,s);
			system("cls");
			Slowsay("“据说那是唯一能击败巨岩魔王的法宝”\n");
			getline(cin,s);
			system("cls");
...
bool shuangfeng() {
	...
	if(Is_first[4]) {
		...
		getline(cin,s);
		Slowsay("“好了，时间不多了，你赶紧走吧，去完成你的使命！”\n");
		getline(cin,s);
		system("cls");
...
void game() {
	if(!Is_save)
	{
		...
		cout<<endl;
		Sleep(500);
		qsay("  1,狂战士：化愤怒为力量，致使自身血量大幅度提升。具有血量极高，防御力较强，攻击力一般的特点");
		cout<<endl<<endl;
		Sleep(1000);
		qsay("  2,圣骑士：作为队伍的核心，可以治愈任何一个队友。具有血量高，防御力极强，攻击力一般的特点");
		Sleep(1000);
		cout<<endl<<endl;
		qsay("  3,刺客：天生的暗杀者，行动迅速，可以利用剧毒攻击敌人，使对方持续扣血。具有血量一般，防御力一般，攻击力极强的特点");
		Sleep(1000);
		cout<<endl<<endl;
		qsay("  4,法师：可以利用火，雷，电元素来攻击敌人，还可召唤石魔来分散敌人注意力，具有血量弱，攻击力爆表，防御力弱的特点");
		Sleep(1000);
		cout<<endl<<endl;
		Slowsay("请选择：");
		HideCursor(1);
		...
		if(MessageBox(NULL,"是否跳过剧情?(新手建议不跳过)","艾诺迪亚5:巨岩魔王的觉醒",MB_YESNO)!=6) {
			...
			Sleep(1000);
			system("cls");
			Slowsay("经过一番深思熟虑，他们选中了一颗光秃秃的没有任何生命迹象的星球\n");
			Sleep(1500);
			system("cls");
			Slowsay("地球\n");
			Sleep(1000);
...
int main() {
	...
	if(ch=='1') {
		...
		system("cls");
		Sleep(2500);
		PlaySound(TEXT("C:\\Users\\Administrator\\Downloads\\Everything I Need.wav"),NULL,SND_ASYNC | SND_LOOP);
		Sleep(1000);
		stop_time=58;
		HideCursor(0);
		Slowsay("后记：\n\n");
